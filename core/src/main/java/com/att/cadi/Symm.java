...
public class Symm {
   ...
   public static Symm obtain(Access access) {
		...
		if(keyfile!=null) {
			...
			try {
				access.log(Level.INFO, Config.CADI_KEYFILE,"points to",file.getCanonicalPath());
			} catch (IOException e1) {
				access.log(Level.INFO, Config.CADI_KEYFILE,"points to",file.getAbsolutePath());
			}
			if(file.exists()) {
				try {
  ...
  public void enpass(String password, OutputStream os) throws IOException {
		...
		if(this.equals(base64) || this.equals(base64url)) { // don't expose randomization
			dos.write(bytes);
		} else {
			Random r = new Random();
			int start = 0;
  ...
  /**
   * Decypt a password into a String
   * 
   * Convenience method
   * 
   * @param password
  ...
  public long depass(String password, OutputStream os) throws IOException {
	  ...
	  ByteArrayOutputStream baos = new ByteArrayOutputStream();
	  ByteArrayInputStream bais =  new ByteArrayInputStream(password.getBytes(),offset,password.length()-offset);
	  decode(bais,baos);
	  byte[] bytes = baos.toByteArray();
	  DataInputStream dis = new DataInputStream(new ByteArrayInputStream(bytes));
	  long time;
	  if(this.equals(base64)) { // don't expose randomization
		  os.write(bytes);
		  time = 0L;
	  } else {
		  int start=0;
